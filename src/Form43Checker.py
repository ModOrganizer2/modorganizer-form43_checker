...
class Form43Checker(mobase.IPluginDiagnose):
    ...
    def description(self):
        return self.tr("Checks plugins (.ESM/.ESP files) to see if any are lower than Form 44 (Skyrim SE).")
    ...
    def fullDescription(self, key):
        ...
        outputString = self.tr("You have one or more plugins that are not form 44. They are:{0}").format(pluginListString)
        outputString += "<br><br>"
        outputString += self.tr("Form 43 (or lower) plugins are modules that were made for Skyrim LE (Oldrim) and have not been "
